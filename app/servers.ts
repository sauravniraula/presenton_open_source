...
export async function startFastApiServer(directory: string, port: number, env: FastApiEnv) {
  // Start FastAPI server
  const fastApiProcess = spawn('env/bin/python', ['server.py', "--port", port.toString()], {
    cwd: directory,
...
export async function startNextJsServer(directory: string, port: number, env: NextJsEnv) {
  // Start NextJS server
  const nextjsProcess = spawn("npm", ["run", "dev", "--", "-p", port.toString()], {
    cwd: directory,
    stdio: ["inherit", "pipe", "pipe"],
    env: { ...env },
  })
