...
import { ThemePreference } from '../services/userPreferences';
...
export const defaultColors= {
  ...
  light_red: {
    ...
    slideBox: '#F3E8E8',
    iconBg:'#F0695F',
    chartColors: ['#F0695F','#450808', '#8F1010', '#C1392F', '#EC5555', '#F49E9E'],
...
const themeSlice = createSlice({
  ...
  reducers: {
    setTheme: (state, action: PayloadAction<ThemeType>) => {
      ...
      state.currentColors = serverColors[action.payload] || defaultColors[action.payload];
    },
    setThemeColors: (state, action: PayloadAction<Partial<ThemeColors> & {theme: ThemeType}>) => {
    ...
    loadSavedTheme: (state, action: PayloadAction<ThemePreference>) => {
...
export const { setTheme, setThemeColors, setLoadingState, loadSavedTheme } = themeSlice.actions;
