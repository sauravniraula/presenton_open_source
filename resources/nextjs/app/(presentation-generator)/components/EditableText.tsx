import { updateSlideDescription, updateSlideBodyString, updateSlideTitle, updateSlideBodyHeading, updateSlideBodyDescription } from '@/store/slices/presentationGeneration';
...
interface EditableTextProps {
    ...
    type: 'title' | 'heading' | 'description-body' | 'description' | 'heading-description' | 'info-heading' | 'info-description';
    content: string;
    isAlingCenter?: boolean;
}

const EditableText = ({ slideIndex, elementId, type, content, bodyIdx = 0, isAlingCenter = false }: EditableTextProps) => {
    ...
    const updateSlide = (type: string, value: string) => {
        ...
        switch (type) {
            case 'title':
                ...
                break;
            case 'heading':
                dispatch(updateSlideBodyHeading({ index: slideIndex, bodyIdx: bodyIdx, heading: value }));
                break;
            case 'description':
                dispatch(updateSlideDescription({ index: slideIndex, description: value }));
                break;
            case 'heading-description':
                dispatch(updateSlideBodyDescription({ index: slideIndex, bodyIdx: bodyIdx, description: value }));
                break;
            case 'description-body':
    ...
    return (
        <>
            {isStreaming ? (
                ...
                >
                    <Typewriter
                        text={content.replace(/\*\*/g, '')} speed={20} />
                </div>
            ) : (
                <TipTapEditor
                    key={content}
