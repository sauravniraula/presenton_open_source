...
import { IconMapper } from '../slide_config';
...
const colors = ['#6453ff', '#22c1dd', '#ff6453', '#ffc122', '#22ddc1', '#c122ff', '#dd22ff', '#ff22c1', '#c1ff22', '#22ffc1'];

const MiniInfoGraphics = ({ slideIndex, itemIndex, chart }: { slideIndex: number, itemIndex: number, chart: Chart }) => {
    const { currentColors } = useSelector((state: RootState) => state.theme);

    const percentage = chart.value.number_type === 'fraction' && chart.value.numerator && chart.value.denominator
        ? (chart.value.numerator / chart.value.denominator) * 100
        : chart.value.percentage || 0;

    return (
        <div>
            {chart.chart_type === 'progress-dial' && <MiniProgressDial color={currentColors.chartColors[0]} percentage={percentage} />}
...
function MiniIconGraphics({ color, icon, percentage }: { color: string, icon: string, percentage: number }) {
            ...
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 60 60">
                ...
                {percentageValue !== 0 && (
                    <circle
                        ...
                        strokeDashoffset={((100 - percentage) / 100) * adjustedCircumference + gap}
                    />
                )}
            </svg>
            <div style={{ color: color }} className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-sm">
...
function MiniProgressDial({ color, percentage }: { color: string, percentage: number }) {
    const percentageValue = percentage > 100 ? 100 : percentage;
...
function MiniRadialProgress({ color, percentage, numerator, denominator, numberType }: { color: string, percentage: number, numerator?: number, denominator?: number, numberType: string }) {
    const percentageValue = percentage > 100 ? 100 : percentage;
    const radius = 20;
    const arcLength = Math.PI * radius;
    const offset = percentageValue === 100 ? 0 : arcLength * (1 - percentageValue / 100);
...
function MiniProgressRing({ color, percentage, numerator, denominator, numberType }: { color: string, percentage: number, numerator?: number, denominator?: number, numberType: string }) {
    const percentageValue = percentage > 100 ? 100 : percentage;
    const radius = 20;
    const circumference = 2 * Math.PI * radius;
    const gap = percentageValue === 100 ? 0 : 2;
            ...
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 60 60">
                ...
                {percentageValue !== 0 && (
                    <circle
                        ...
                        cy="30"
                        strokeDasharray={`${adjustedCircumference}`}
                        strokeDashoffset={((100 - percentageValue) / 100) * adjustedCircumference + gap}
                    />
                )}
            </svg>
...
function MiniProgressBar({ color, percentage }: { color: string, percentage: number }) {
    const percentageValue = percentage > 100 ? 100 : percentage;
    return (
        <div
...
function MiniTextInfographic({ iconBg, numerical }: { iconBg: string, numerical: number }) {
